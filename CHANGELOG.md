...
## Moteur 3.1.11 · dépôt du 22 septembre 2026

**L’arbre de l’UNIL est inchangé** (`cbdb4863aed9f778`). Le moteur ne change que par un commentaire, mais ce
commentaire est dans son bloc : l’empreinte bouge, donc le numéro aussi.

**Une date fausse, répétée dans quatre fichiers depuis des mois.** `new Date("03.12.2026")` rend le **12** mars,
pas le 11. Le commentaire du moteur le disait faux, le journal le recopiait, et les deux guides le répétaient, en
français et en anglais. La leçon qu’ils enseignent reste juste -- une date suisse acceptée puis relue à l’envers --
seul le jour était faux. Vérifiable : `TZ=UTC node -e 'console.log(new Date("03.12.2026").toISOString())'`.

**Et `SECURITY.md` annonçait l’isolation entre onglets trop absolument.** Un onglet ouvert par `window.open`
depuis la page reçoit du navigateur une **copie** du `sessionStorage` de son parent, prise à l’ouverture. Ce n’est
pas une synchronisation, les deux divergent ensuite ; mais c’est une exception réelle, et elle est maintenant
écrite dans la page qui parle de sécurité.

**Le harnais passe de 702 à 702 assertions**, aucune n’étant nécessaire ici : les deux corrections portent sur des
phrases, et le harnais gèle déjà l’empreinte du bloc qui en contient une.
## Moteur 3.1.10 · dépôt du 22 septembre 2026

**Le comportement du moteur n’a pas changé d’une ligne**, et **l’arbre de l’UNIL non plus** (`cbdb4863aed9f778`).

**Le vrai questionnaire n’était essayable nulle part.** On pouvait le lire, pas l’exécuter : le fichier qu’on
double-clique porte un exemple de trois questions, et l’arbre de l’UNIL n’existait que comme objet JavaScript. Le
harnais, lui, compose cette page depuis toujours et la vérifie -- il ne le disait à personne. Le README donne
maintenant le geste, et une assertion tient les deux ensemble : l’instruction ne peut plus dériver du test.

**Trois défauts de contrôle, tous de ma main** :

- **Dans une politique, la PREMIÈRE directive gagne**, pas la dernière : « script-src 'none'; script-src
  'unsafe-inline' » laisse `'none'`. Mon analyseur écrasait, donc il rendait vert un fichier que le navigateur
  laisse blanc.
- **Un script `nomodule` n’est pas exécuté** par un navigateur moderne. Je ne regardais que `type`, et le harnais
  exécutait le corps extrait quand même.
- **Mon faux `querySelector` acceptait tout ce qui contenait le fragment** : le sélecteur « .absent
  iframe[data-aigo] » passait, alors qu’un navigateur ne trouve aucun cadre.

**Et l’accueil, désigné comme l’axe le plus faible par huit noteurs** : ce dépôt dit désormais en une phrase ce
qu’il est (ni service en ligne, ni bibliothèque), et développe les sigles qu’il lançait sans un mot -- LLM, la
LPrD-VD vaudoise. La borne du README passe de 115 à 117 lignes, cinq paragraphes ayant été repliés d’abord.

**Le harnais passe de 701 à 702 assertions.**
## Moteur 3.1.9 · dépôt du 22 septembre 2026

**Le comportement du moteur n’a pas changé d’une ligne**, et **l’arbre de l’UNIL non plus** (`cbdb4863aed9f778`).

**Le contrôle de politique d’exécution suit maintenant la spécification, et non ma convenance.** Trois trous, dont
deux de ma main la veille :

- **Dès qu’un nonce ou une empreinte figure dans la directive, un navigateur IGNORE `'unsafe-inline'`.** Les
  accepter ensemble rendait vert un fichier que Chrome laisse blanc. Et un nonce doit être porté par le script
  **du moteur**, pas par un autre de la page.
- **Deux politiques se cumulent**, la plus stricte gagnant : je n’en lisais qu’une, donc une seconde balise qui
  interdisait tout passait inaperçue.
- **Un `<fieldset disabled>` éteint ce qu’il contient.** Le contrôle ne regardait que l’élément lui-même : un
  bouton éteint par son parent passait. Aucun attribut `disabled` n’a de raison d’être dans cette page.

**Et un script dont le `type` n’est pas du JavaScript est inerte dans un navigateur**, pendant que le harnais en
extrayait le texte et l’exécutait quand même : la page morte, la course verte. Les types admis sont désormais
ceux qu’un navigateur exécute.

**Le harnais passe de 700 à 701 assertions.**
## Moteur 3.1.8 · dépôt du 22 septembre 2026

**Le comportement du moteur n’a pas changé d’une ligne**, et **l’arbre de l’UNIL non plus** (`cbdb4863aed9f778`).

**La page de contrôle mourait sur le second fichier.** Une personne vérifie un fichier, puis en dépose un autre :
la section de mise à jour compare les deux routages, et elle relit le nouvel arbre **hors du garde qui protège
tout le reste**. Un arbre qui lève à la lecture arrêtait donc la page entière, sans bannière et sans un mot. Le
commentaire juste au-dessus promet pourtant l’inverse, et il le tenait : pour le premier fichier seulement.

C’est l’assertion des quatre gestes, posée la veille, qui l’a trouvé : personne n’avait jamais déposé deux
fichiers de suite. Le harnais appelait `analyse()` une fois, et la deuxième fois n’existait pour personne.

**Et cette assertion était elle-même trop lâche :** elle acceptait le mot « question », si bien que remplacer
toute l’analyse par `out.textContent = 'question'` la laissait verte. Elle exige désormais le décompte que la page
calcule et le verdict qu’elle rend. J’avais écrit, contre le défaut de la semaine, une assertion qui en souffrait.

**Le harnais passe de 699 à 700 assertions.**
## Moteur 3.1.7 · dépôt du 22 septembre 2026

**Le comportement du moteur n’a pas changé d’une ligne**, et **l’arbre de l’UNIL non plus** (`cbdb4863aed9f778`).

**La page de contrôle est enfin pilotée par ses propres boutons.** Le harnais appelait `analyse()` en direct :
tout le câblage de `check.html` -- le bouton qui vérifie un bloc collé, le sélecteur de fichier, la zone de dépôt --
n’était exercé par rien. C’est par là qu’étaient passés un identifiant renommé (navigateur mort, harnais vert) et
un `disabled` sur le bouton. Une assertion fait désormais **les quatre gestes d’une vraie personne** et lit ce
qu’elle voit : coller un bloc et cliquer, cliquer sans rien avoir collé, choisir un fichier (par `FileReader`,
comme le navigateur), cliquer dans la zone de dépôt. Les quatre mutations correspondantes meurent, chacune sur
son geste.

**Pourquoi c’est autre chose qu’une assertion de plus.** Trois mesures dans la journée ont donné 7,26, puis 7,5,
puis 7,25, pendant que le harnais passait de 688 à 698 assertions : **ajouter des assertions de texte n’a pas
fait monter l’axe du contrôle, et l’a fait descendre.** Une assertion qui cherche un fragment de texte est bon
marché à écrire et bon marché à défaire. Les noteurs ont nommé deux fois le remède : éprouver le parcours, pas
le texte. Faute de pouvoir ouvrir un navigateur -- ce dépôt n’a aucune dépendance, par choix -- c’est le faux DOM
qui joue les gestes, et il les joue vraiment.

**Le harnais passe de 698 à 699 assertions.**
## Moteur 3.1.6 · dépôt du 22 septembre 2026

**Le comportement du moteur n’a pas changé d’une ligne**, et **l’arbre de l’UNIL non plus** (`cbdb4863aed9f778`).

**La 3.1.3 a été re-notée 7,5** (contre 7,26 pour la 3.1.2), l’axe du contrôle passant de 6 à 6,5. Le même noteur
a nommé la couche suivante, trois défauts reproduits :

- **« Publiable » sur une page que le navigateur laisserait blanche.** Une adaptation où `'unsafe-inline'` avait
  sauté du `script-src` obtenait un feu vert ; le moteur EST un script en ligne, et le conteneur restait vide.
  Cette page lisait le contenu sans jamais éprouver la politique d’exécution du fichier. C’est la pire forme du
  défaut de la semaine : contrôler un texte que personne ne verra. Le validateur refuse désormais, et il le dit.
- **`disabled` ajouté au bouton passait le harnais**, qui ne vérifiait que la présence de l’identifiant. Aucun des
  huit éléments câblés n’est livré désactivé, et une assertion le tient.
- **Le fragment d’iframe que `docs/INTEGRATE.md` fait recopier n’était exécuté par personne.** Le vider laissait le
  harnais vert, parce qu’il cherchait trois fragments de texte. Il est maintenant lu dans le guide et exécuté :
  une hauteur demandée doit arriver au cadre, et seul le message du moteur doit y parvenir.

**Mon propre correctif de CSP était trop court, et la re-notation l’a montré dans l’heure** : il acceptait un
nonce sans vérifier qu’un script le porte, et ignorait `script-src-elem`, qui prime sur `script-src` pour les
balises. Deux mutations obtenaient « Publiable » pendant que Chrome bloquait tout. Les trois directives sont
maintenant lues dans l’ordre du navigateur, un nonce n’autorise que les scripts qui le portent, et **`check.html`
s’applique sa propre règle** : sa CSP passée à `'none'` laissait le harnais vert, la page morte.

**Le harnais passe de 694 à 698 assertions.** Les quatre nouvelles tuent les mutations exactes des noteurs.
## Moteur 3.1.4 · dépôt du 22 septembre 2026

**Le comportement du moteur n’a pas changé d’une ligne**, et **l’arbre de l’UNIL non plus** (`cbdb4863aed9f778`).

**Cinq lecteurs de plus ont noté la 3.1.2** (huit en tout, moyenne 7,26, même axe faible). Quatre trouvailles
neuves, dont deux que j’avais introduites la veille :

- **Une phrase fausse de mon fait.** Les entrées 3.1.2 et 3.1.3 disaient « même bloc, même empreinte » pendant
  que le registre enregistrait quatre empreintes de bloc différentes. Le numéro de version est écrit DANS le bloc :
  le bloc change à chaque version. Ce qui ne change pas, c’est le comportement. Les deux entrées le disent
  maintenant.
- **`tools/anglais.mjs --render` rendait l’arbre d’autrui sous l’identité de l’UNIL** : titre « The AI_GO
  questionnaire of UNIL », notre empreinte quelle que soit la vraie, « The 20 possible routes » pour un arbre
  qui en a deux, et notre adresse en bas. C’est la falsification que le commentaire de la constante déclare pire
  que le silence. Tout est dérivé de l’arbre reçu, et une assertion vérifie qu’il ne reste rien de nous.
- **L’assertion « uses fieldset » ne vérifiait jamais la balise `fieldset`**, seulement la classe et le premier
  enfant. Remplacer les deux `el('fieldset')` par `el('div')` passait, empreinte régénérée comprise ; et
  `docs/DEMARRER.md` promet à une institution tierce que le harnais contrôle ce regroupement, phrase qu’elle
  recopierait dans une déclaration d’accessibilité.
- **Le harnais mourait sur le fichier d’une adoptante.** Lancé dans un dossier ne contenant que ce que
  `docs/INTEGRATE.md` fait télécharger, il s’arrêtait sur une trace de pile ENOENT cinq cents assertions avant la
  fin, sans un seul FAIL, alors que ce guide promet qu’il « checks what it still can ». Il rend 565/565 et nomme
  les quatre pages qu’il n’a pas lues. Une assertion relance le harnais livré dans ce dossier nu.

**Le harnais passe de 692 à 694 assertions.**
## Moteur 3.1.3 · dépôt du 22 septembre 2026

**Le comportement du moteur n’a pas changé d’une ligne**, et **l’arbre de l’UNIL non plus**
(`cbdb4863aed9f778`). Seul le numéro qu’il déclare a bougé ; comme ce numéro est écrit dans le bloc, l’empreinte
du bloc bouge avec lui, et le registre l’enregistre. Un numéro neuf pour la raison des deux précédents : le registre empreinte `check.html` et
`test/run.mjs`, qui changent ici.

**Huit noteurs indépendants ont mesuré la 3.1.2** sur quatre axes : accueil du grand public, reprenabilité par
une institution tierce, exactitude factuelle, solidité du contrôle. Trois notes globales identiques, 7,25, et le
même axe faible : **le contrôle, noté 6, 6 et 7**. Leur reproche tient en une phrase : plusieurs garanties
dépassaient ce que les contrôles prouvaient. Voici les leurs, toutes reproduites avant correction.

- **Le validateur disait « Publiable » d’une page qu’il n’avait pas fini de lire.** L’énumération des chemins
  s’arrête à une borne ; le drapeau existait et s’écrivait dans un paragraphe tout en bas, la phrase de tête
  l’ignorait. Un arbre cyclique ou très large recevait « Publiable, rien à signaler ». Il reçoit désormais
  « Pas de verdict ».
- **La comparaison de mise à jour affirmait « Le routage n’a pas bougé » sur une énumération tronquée.**
  `routingDiff` calculait déjà le drapeau, avec un commentaire disant qu’il invalide la comparaison : l’affichage
  l’ignorait. Le code promettait dans son commentaire ce qu’il ne faisait pas.
- **Renommer `id="pasteGo"` laissait le harnais tout vert** pendant que le navigateur levait une erreur et ne
  rendait aucun verdict. Le faux DOM fabriquait ce qu’on lui demandait ; il exige maintenant que chaque
  identifiant cité par le script existe dans le balisage.
- **Une aide non traduite échappait au contrôle quand c’était une liste.** `help: ["Texte français",
  { items: [...] }]` dans un arbre FR/EN donnait zéro avertissement et un verdict vert, pendant que l’interface
  anglaise servait cette aide en français.
- **Une assertion promettait une procédure et ne cherchait que deux mots.** Supprimer tout le paragraphe
  « Updating the engine » du guide laissait le harnais vert ; elle exige désormais les sept gestes.

**Et trois écarts relevés sur le README** par le même tour : `tools/anglais.mjs` cité sous licence mais absent de
l’inventaire du dépôt, « treize textes à remplacer » réconcilié avec les sept champs détaillés plus bas, et
`docs/TREE.en.md` nommé là où l’outil qui le rend l’est aussi.

**Le harnais passe de 688 à 692 assertions.** Les six nouvelles ont toutes tué leur mutant, y compris les deux
mutations exactes que les noteurs avaient fait passer.
## Moteur 3.1.2 · dépôt du 22 septembre 2026

**Le comportement du moteur n’a pas changé d’une ligne**, et **le texte de l’arbre de l’UNIL
est intact** (`cbdb4863aed9f778`). Un numéro neuf pour la raison de la 3.1.1 : le registre empreinte `check.html`
et `test/run.mjs`, qui changent ici.

**Ce que dix auditeurs ont trouvé dans la 3.1.1**, huit constats qui ont survécu à une réfutation adverse, dont
cinq dans le travail de la veille :

- **Le schéma anglais étiquetait ses arêtes « yes » et « no ».** Deux titres de cet arbre sont des groupes
  nominaux et non des questions, « Impact assessment with the DPO » et « Data anonymisation », et ce sont
  justement les deux nœuds dont une branche élargit les outils permis. Les arêtes portent désormais le détail de
  chaque réponse, tel que l’arbre l’écrit en anglais pour ses 14 réponses binaires : rien de neuf n’est écrit,
  l’ambiguïté tombe.
- **Aucun texte anglais du dépôt ne disait que la page en ligne fait tourner une implémentation antérieure.** Le
  français le dit deux fois, dont une en gras ; la parenthèse avait sauté sur la même phrase d’invitation. Elle
  est rétablie dans la section « In English » du README et sur `docs/TREE.en.md`.
- **« Le moteur porte 33 clés » suivi de l’énumération de 31.** Les deux absentes étaient `record` et
  `consultedOn`, celles de la feuille imprimable : une institution qui recopiait les clés nommées obtenait de
  l’anglais sur son papier, invisible à l’écran où ce bloc est caché.
- **Le compte d’assertions ne chaînait plus.** Trois entrées successives repartaient de 498. Le harnais ne relisait
  que le nombre d’arrivée, le départ vivant dans un groupe non capturant ; il relit maintenant les deux, sur les
  quatre entrées les plus récentes. Sous la 3.0.18 subsiste un écart d’une unité, écrit avant que le harnais ne
  relise son compte : ces chiffres publiés ne sont pas réécrits pour faire verdir un test.
- **Le harnais rougissait sur la typographie d’une autre institution.** Un tiret cadratin dans la question d’une
  adoptante faisait échouer une règle de style qui est la nôtre, alors que `docs/INTEGRATE.md` promet qu’il
  vérifie « ce qu’il peut encore ». Mesuré : 635/636 avant, 636/636 après.
- **`docs/DEMARRER.md` écrivait « le moteur ne lit aucun des six ».** Il en lit deux, `data-lang` et
  `data-heading-level`, et une assertion du harnais le prouve depuis longtemps.
- **`CONTRIBUTING.md` annonçait un README d’au plus 110 lignes** quand le harnais en autorise 115. Les deux
  chiffres sont désormais tenus ensemble par une assertion.
- **`docs/ARBRE.md` ne citait l’avertissement qu’après les sept résultats**, à la troisième personne, là où sa
  jumelle anglaise l’a en tête. Il y est aussi.

**Les outils sont listés avec leurs liens** sur `docs/TREE.en.md` : l’arbre les porte, le questionnaire en ligne
les rend cliquables, la page les taisait.

**Le harnais passe de 685 à 688 assertions.**
## Moteur 3.1.1 · dépôt du 22 septembre 2026

**Le moteur n’a pas changé d’une ligne** (bloc identique à celui de la 3.1.0), et **le texte de l’arbre de
l’UNIL et ses 20 chemins non plus** (empreinte de contenu `cbdb4863aed9f778`). Ce qui change, ce sont le
validateur, le harnais et les pages. **Un numéro neuf quand même** : le registre empreinte `check.html` et
`test/run.mjs`, et republier deux fichiers différents sous le numéro 3.1.0 referait précisément ce que la
3.1.0 a été ouverte pour arrêter.

**Le questionnaire se lit en anglais.** La section « In English » du README envoyait un lecteur anglophone
sur `docs/ARBRE.md`, qui est en français. `docs/TREE.en.md` donne le même questionnaire dans l’anglais de
l’arbre : ses 109 chaînes `en:`, mot pour mot, rendues par `tools/anglais.mjs` et comparées octet pour octet
par le harnais, qui vérifie aussi que le diagramme anglais dessine les 22 arêtes du routage gelé. **Pas un mot
n’y est traduit.** Les trois sections de commentaire de la page française, les sigles, les cinq remarques et
les phrases de lecture des 20 chemins, n’y sont pas : les traduire serait écrire un texte neuf sur le droit
suisse, ce qui revient aux autrices. La page le dit, et nomme les trois sigles qu’elle n’explique pas.

**Le questionnaire d’exemple se nomme avant de se faire gronder.** Qui télécharge `ai-go.html`, le fichier que
le README appelle « le moteur », et le dépose dans `check.html` recevait « Cohérent, mais NON publiable :
13 point(s) à compléter » au-dessus d’un tableau rouge de treize lignes. Le fichier n’est pas cassé : c’est
le questionnaire d’exemple livré avec le moteur, et treize blancs, c’est ce qu’il doit avoir. La phrase qui
l’expliquait était la treizième ligne du tableau ; elle est maintenant la première, et elle dit toujours
« NON publiable ». **L’exemple se reconnaît à l’empreinte de ses questions**, celle que le moteur calcule
déjà, et jamais à son identifiant : une institution qui a écrit ses questions retrouve le verdict ordinaire
même si elle a oublié de renommer le fichier, et la renommer seule ne suffit pas à en faire le sien. Cette
seconde version doit son existence à un audit externe, qui a montré que la première mentait à qui avait écrit
ses questions sans changer l’identifiant.

**Le verdict de tête est sorti du rendu.** C’est la seule ligne que beaucoup liront, et elle se calculait entre
deux `appendChild`, hors d’atteinte de toute assertion. `verdictOf` est désormais exportée, et ses sept cas
sont éprouvés dans les deux langues.

**Le harnais passe de 673 à 685 assertions.** Chacune de celles ajoutées depuis la 3.1.0 a été vérifiée en
retirant son correctif dans une copie, pour s’assurer qu’elle tue son mutant ; six l’ont été à la main, et
trois des premières versions ne tuaient rien.

## Moteur 3.1.0 · dépôt du 15 septembre 2026

**Le texte de l’arbre de l’UNIL et ses 20 chemins sont inchangés** (empreinte de contenu `cbdb4863aed9f778`).

**Pourquoi un numéro neuf.** La 3.0.19 a été publiée le 9 septembre, puis republiée plusieurs fois avec un
moteur dont le comportement avait changé : plusieurs fichiers différents ont ainsi porté le même numéro et
la même date, et qui cite « AI_GO 3.0.19 » ou vérifie une empreinte ne pouvait pas savoir lequel il tenait.
Un **mineur** et non un correctif, parce que ce qui change se voit à l’écran : cliquer une réponse ne fait
plus avancer, seul « Continuer » le fait, à la souris comme au clavier.

**Ce qu’une institution ayant déjà publié doit faire.** Remplacer le bloc moteur, et rien d’autre. Les empreintes
du moteur changent ; celle du contenu ne bouge pas.

**Le harnais passe de 665 à 673 assertions.** Chacune de celles ajoutées depuis la 3.0.19 a été vérifiée en
retirant son correctif dans une copie, pour s’assurer qu’elle tue son mutant : une assertion qui ne meurt jamais
ne protège rien.

**Ce que la personne repart avec.** Une chercheuse a fait le parcours en vrai, pris six décisions, obtenu son
verdict, et elle est repartie les mains vides : ni ses réponses, ni la date, rien à joindre à un plan de gestion
de données ou à une commission d’éthique. Le moteur portait pourtant déjà une règle `@media print` écrite pour cet
usage : le besoin avait été vu, ce qui s’imprimait n’y répondait pas. **Le résultat porte désormais un
récapitulatif des questions posées, des réponses données et de la date**, invisible à l’écran où elle les a déjà
sous les yeux, écrit sur le papier qu’elle joindra. Tout ce qu’il dit vient de l’arbre : ce sont les mots de
l’institution, pas les nôtres. Deux phrases d’interface nouvelles, `record` et `consultedOn` ; le moteur en
porte trente-trois.

**Deux frottements que quatre visiteurs non techniques ont trouvés ensemble.** Le fichier téléchargé s’ouvre
coiffé d’un encadré « Aperçu : non publiable » listant treize textes à remplacer, juste après que le guide a promis
« un exemple bilingue de trois questions » : tous les quatre ont conclu que le fichier était cassé. L’encadré est
annoncé, et dit pour ce qu’il est : une liste de courses. Et la contradiction de licence du billet cité en tête
vivait soixante-dix lignes plus bas que le lien ; elle est désormais à côté de lui.

**La section anglaise ne nommait pas le seul document écrit pour qui reprend l’outil.** Le README annonce que
« l’étape 1 prend des semaines, les étapes 2 à 4 une demi-journée » : l’étape 1 est `docs/DEMARRER.md`, 272 lignes en
français, et les étapes 2 à 4 sont `docs/INTEGRATE.md`, 213 lignes en anglais. Le dépôt avait traduit la demi-journée
et gardé les semaines. Pire, `INTEGRATE.md` dit en anglais que ce dont le lecteur a besoin est en français : le
panneau est dans sa langue et désigne un cul-de-sac. La section anglaise nomme désormais la feuille de départ, dit
qu’elle est en français, et annonce que le moteur accepte une troisième langue d’interface.

**La règle la plus visible du validateur n’était tenue par rien.** `check.html` exporte `preferred` avec un
commentaire qui promet l’épreuve : « Exported so the harness can assert the rule rather than a screenshot of it ».
Le harnais ne le mentionnait **nulle part**, et le `documentElement` du faux DOM n’a pas de `getAttribute`, si bien
qu’un appel serait retombé sur le premier garde-fou et aurait rendu l’anglais quoi qu’on lui demande : n’importe
quelle assertion écrite à la légère serait passée. Neuf lecteurs sont désormais éprouvés, dont le poste alémanique
et le tessinois, qui ne demandent aucune des deux langues de la page et la trouvent dans celle que son balisage
déclare. Vérifié en cassant le repli dans une copie : l’assertion meurt sur trois témoins.

**Où lire le détail.** Les corrections des 10 et 15 septembre sont consignées dans l’entrée 3.0.19 ci-dessous,
qui a servi de journal courant tant que le numéro n’avait pas bougé. Les plus visibles : le clic qui pouvait
retourner un verdict, l’empreinte du moteur qui avait cessé de couvrir sa feuille de style, `destroy()` qui rendait
sa clé à chaque appel, un seul conteneur qui prend désormais l’historique, et la vitrine du dépôt reprise pour un
...
- **Une date de relecture écrite à la suisse était acceptée, puis lue à neuf mois près.** `03.12.2026` ne passait
  ...
  **12 mars**. Le fichier était déclaré publiable, imprimait « Relu le 03.12.2026 » sous chaque résultat, et
...
- **Deux mécanismes d’avancée se recouvraient à l’écran, et le verdict pouvait s’inverser.** Un clic de souris
  sur l’étiquette d’une réponse avançait tout seul, et le bouton « Continuer » restait dessiné dessous, à
  l’endroit exact d’une carte de la question suivante. Qui faisait le geste que l’interface enseigne, choisir
  puis Continuer, cochait une case fantôme : mesuré, q1 répondu « oui, mes données concernent des individus »
  et verdict rendu « pas de données personnelles, utilisation libre, LLMs commerciaux ». Les deux s’excluent, et
  c’est le clic qui part : au clavier la flèche sélectionne sans avancer, donc « Continuer » est la seule sortie.
  Souris et clavier font désormais le même geste, et aucun des 20 chemins gelés ne bouge.
- **Le README promettait une question posée à toutes les branches.** Elle ne l’est que sur six chemins sur vingt
  et qui a justement un accord de confidentialité sur un jeu sensible, le cas que le README met en avant, n’est
  jamais interrogé dessus et lit « sans restriction de partage ».
- **La page d’accueil ne disait nulle part que l’outil n’est pas un avis juridique**, alors que le billet qu’elle
  cite le dit et que le questionnaire l’affiche sous chaque résultat. C’est la première chose qu’on cherche.
- **« voir Le dépôt » était du texte brut** dont la réponse se trouvait soixante-treize lignes plus bas, et le
  commit unique n’était expliqué nulle part : le registre où vérifier l’empreinte d’une copie est maintenant
  nommé là où le lecteur regarde.
- **La seule image du README montrait le milieu d’un formulaire à cocher, jamais la réponse.** C’est l’élément
  auquel un visiteur accorde le plus de ses quinze premières secondes, et il en retirait « formulaire
  administratif », pas « outil qui me donne une réponse » ; sur téléphone, réduite d’un facteur quatre, il n’en
  retirait rien. Elle montre désormais l’écran de résultat, en paysage, et son alternative textuelle nomme ce
  qu’on y voit au lieu de la provenance du fichier.
- **Quatre des sept corrections de la veille ne tenaient pas.** Une re-mesure par les mêmes quatre visiteurs les a
  rejouées avant de chercher du neuf, et c’est encore la consigne qui a le plus rapporté. La pire : la phrase
  corrigée sur les six chemins sur vingt était arrivée **cassée**, deux deux-points et un membre de phrase orphelin,
  dans le premier paragraphe qui explique ce que fait l’outil. Les quatre visiteurs ont buté dessus.
- **Le dépôt se contredisait sur le point qu’il venait de corriger.** Le clic-qui-avance a été retiré du moteur, et
  trois textes l’enseignaient encore : le commentaire en tête de la question à choix unique, et surtout les deux guides, qui
  disaient à une institution de **déclarer un écart WCAG 3.2.2** que le moteur ne commet plus.
- **L’avertissement juridique n’existait qu’en français.** La section « In English », seule chose que lit un
  visiteur anglophone et seule phrase qui voyage dans l’encadré de GitHub, promettait une réponse sans un mot de
  réserve. Elle le porte désormais.
- **Le lien « Le dépôt » ne menait nulle part **: GitHub ne résout pas une ancre accentuée. Il est encodé.
- **La borne du README passe de 110 à 115 lignes, et la raison est écrite dans l’assertion** : quatre lignes de
  gras ont été coupées ailleurs d’abord, et ce qui restait était l’avertissement juridique, la contradiction de
  licence et le registre des empreintes. Une borne est un garde-fou contre l’enflure, pas une fin ; couper avant
  de la lever.
- **Le harnais passe de 498 à 665 assertions**, et vérifie désormais ses propres chiffres : le compte qu’il
