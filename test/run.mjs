...
/* UNE PAGE ABSENTE N'EST PAS UNE PANNE, C'EST UNE PAGE ABSENTE. Lance sur le
 * fichier d'une institution qui n'a pris que ai-go.html, check.html et ce
 * harnais -- ce que docs/INTEGRATE.md lui dit de prendre -- il mourait sur une
 * trace de pile ENOENT, cinq cents assertions avant la fin, sans un seul FAIL,
 * alors que ce guide promet qu'il « checks what it still can ». Le fichier
 * manquant est desormais rendu vide ET NOMME dans ce que la course n'a pas
 * regarde : un silence qui se dit n'est pas un silence. */
const absent = [];
/* Ce que cette copie porte : une assertion qui lit une page absente ne se juge
 * pas sur du vide, elle se passe, et la page est nommee dans le bilan. */
const porte = f => read(f).length > 0;
const read = f => {
  try { return readFileSync(join(ROOT, f), 'utf8'); }
  catch (e) {
    if (e.code !== 'ENOENT') throw e;
    if (absent.indexOf(f) < 0) absent.push(f);
    return '';
  }
};
...
const skipped = [];
/* Les pages absentes rejoignent la liste juste avant qu'elle ne soit dite. */
const nameMissing = () => { if (absent.length) skipped.push('les ' + absent.length + ' page(s) que cette copie ne porte pas : ' + absent.join(', ')); };
...
if (onlyDelivered('the shipped example, read as a tree')) {
  ...
  // que la signature règle et cinq qui demandent d'écrire son propre contenu.
  {
    ...
    if (porte('docs/DEMARRER.md')) ok(`filling the signature leaves ${left} points, which is what docs/DEMARRER.md says`,
...
// against the projection the code actually hashes.
{
  ...
  if (porte('docs/ARBRE.md') && porte('docs/INTEGRATE.md'))
...
// So the whole range is walked here, at both edges of the word, every time.
{
  ...
  if (porte('SECURITY.md')) ok('a letter glued to the name makes another word, and the guide says so',
...
section(() => {
  ...
  /* LE CLIC DE SOURIS CHOISIT, IL N'AVANCE PAS. Il avancait, et « Continuer »
   * restait dessine dessous a l'endroit d'une carte de la question suivante :
   * le geste que l'interface enseigne cochait une case fantome, et le verdict
   * pouvait s'inverser. Souris et clavier font le meme geste. */
  yes.checked = true; yes.fire('change'); yes.fire('click', { detail: 1 });
  flush();
  ok('a mouse click on a radio selects without navigating, exactly like the keyboard',
    inst.node === 'q1', String(inst.node));
  root.querySelector('.aigo-btn-primary').fire('click');
  flush();
  ok('and Continue is what carries the answer, from either device', inst.node === 'q2');
  ...
  ok('a checkbox question uses fieldset and a legend that repeats the question',
    ...
    // ET LA BALISE ELLE-MEME. Cette assertion lisait la classe et le tagName du
    // PREMIER ENFANT, jamais celui de l'element : remplacer les deux
    // el('fieldset', ...) par el('div', ...) la laissait verte, pendant que
    // docs/DEMARRER.md promet a une institution tierce que le harnais controle
    // ce regroupement -- phrase qu'elle recopierait dans sa declaration
    // d'accessibilite. Trouve par un noteur externe.
    !!root.querySelector('.aigo-fieldset') &&
    root.querySelector('.aigo-fieldset').tagName === 'fieldset' &&
  ...
  /* ---- CE QUE LA PERSONNE REPART AVEC -------------------------------------- */
  // Une chercheuse a fait le parcours, pris six décisions, obtenu son verdict, et
  // elle est repartie les mains vides : ni ses réponses, ni la date, rien à
  // joindre à un plan de gestion de données ou à une commission d'éthique. Le
  // moteur portait déjà une règle @media print écrite pour cet usage : le besoin
  // avait été vu, ce qui s'imprimait n'y répondait pas.
  {
    const asked2 = {
      id: 'record-example', langs: ['fr'], defaultLang: 'fr', start: 'q1',
      publisher: { name: 'Example UAS', domains: ['example-uas.ch'] },
      review: { by: 'Service juridique', date: '2026-06-01' },
      jurisdiction: 'CH-VD', legalBasis: ['LPD'], disclaimer: 'Pas un avis juridique.',
      steps: [{ id: 's', name: 'Étape' }],
      nodes: {
        q1: { type: 'single', step: 's', title: 'Des personnes sont-elles concernées ?',
          answers: [{ value: 'oui', label: 'Oui, des individus', to: 'q2' },
                    { value: 'non', label: 'Non', result: 'r' }] },
        q2: { type: 'single', step: 's', title: 'Le partage est-il restreint ?',
          answers: [{ value: 'oui', label: 'Oui, par un accord', result: 'r' },
                    { value: 'non', label: 'Non', result: 'r' }] }
      },
      results: { r: { level: 'info', step: 's', title: 'Un résultat', solution: { text: 'Faites ceci.' } } },
      links: {}
    };
    const rr = new N('div'); body.appendChild(rr);
    const ir = AI_GO.mount(rr, asked2, { lang: 'fr', storageKey: null, history: false, mode: 'preview' });
    const answer = v => {
      const box = rr.querySelectorAll('input').find(x => x.value === v);
      box.checked = true; box.fire('change');
      rr.querySelector('.aigo-btn-primary').fire('click');
    };
    answer('oui'); answer('non');
    ok('the walk reaches a result, which is where the record is drawn', ir.result === 'r');

    const rec = rr.querySelector('.aigo-record');
    ok('the result carries a record of the answers given',
      !!rec, rec ? 'présent' : 'absent');
    const dit = rec ? rec.textContent : '';
    /* CE QU IL DOIT PORTER : les questions posées, les réponses choisies, et la
     * date. Chacune séparément : un bloc qui ne porte que la moitié de cela ne
     * prouve rien à une commission. */
    const PORTE = [
      ['le titre du bloc', 'Vos réponses'],
      ['la première question', 'Des personnes sont-elles concernées'],
      ['la réponse donnée', 'Oui, des individus'],
      ['la seconde question', 'Le partage est-il restreint'],
      ['la seconde réponse', 'Non'],
      ['la date du jour', new Date().toISOString().slice(0, 10)]
    ];
    const absents = PORTE.filter(([, t]) => dit.indexOf(t) < 0);
    ok(`the record carries the ${PORTE.length} things a committee needs to read it`,
      absents.length === 0, absents.map(x => x[0]).join(' | '));
    /* ET IL NE SE VOIT PAS A L ECRAN, ou la personne a deja tout sous les yeux :
     * la feuille du moteur le cache, et ne le montre qu'a l'impression. */
    const sheet = CHECK.engineBlock(html);
    ok('and it is hidden on screen, shown only on paper',
      /\.aigo-record \{ display: none; \}/.test(sheet) &&
      /@media print \{[\s\S]*?\.aigo-record \{ display: block/.test(sheet));
    // Une question sautée n'a pas de réponse : elle ne figure pas au procès-verbal.
    ok('a question the path never asked is not written into the record',
      dit.indexOf('q3') < 0 && dit.split('Vos réponses').length === 2);
  }
...
section(() => {
  ...
  /* ---- LA PAGE PILOTEE PAR SES PROPRES BOUTONS ----------------------------
   * Le harnais appelait analyse() en direct : le cablage de cette page n'etait
   * exerce par rien, et c'est par la que sont passes un identifiant renomme
   * (navigateur mort, harnais vert) et un `disabled` sur le bouton. On fait donc
   * les gestes d'une vraie personne et on lit ce qu'elle voit. */
  {
    const vu = () => ids.out.textContent;
    /* 1. Je colle le bloc de contenu et je clique sur le bouton. */
    ids.out.textContent = '';
    /* Du marqueur de debut au marqueur de fin, conteneur compris : c'est ce que
     * la page demande. Coller le seul bloc de contenu rend « ce fichier ne
     * monterait pas son questionnaire », message juste qui contient le mot
     * « question » -- et c'est ce mot que la premiere version prenait pour un
     * verdict. */
    ids.paste.value = html.slice(html.indexOf('AI_GO-BEGIN'), html.indexOf('AI_GO-END') + 10);
    ids.pasteGo.fire('click');
    const apresColle = vu();
    /* 2. Je clique sans rien avoir colle : la page ne doit pas repartir. */
    ids.out.textContent = 'TEMOIN';
    ids.paste.value = '   ';
    ids.pasteGo.fire('click');
    const apresVide = vu();
    /* 3. Je choisis un fichier : le vrai chemin passe par FileReader. */
    ids.out.textContent = '';
    const vraiFR = globalThis.FileReader;
    globalThis.FileReader = function () {
      this.readAsText = (f) => { this.result = f.texte; this.onload(); };
    };
    ids.file.files = [{ texte: html }];
    ids.file.fire('change');
    globalThis.FileReader = vraiFR;
    const apresFichier = vu();
    /* 4. Je clique dans la zone de depot : elle doit ouvrir le selecteur. */
    let ouvert = 0;
    const vraiClick = ids.file.click;
    ids.file.click = () => { ouvert++; };
    ids.drop.fire('click', { target: ids.drop });
    ids.file.click = vraiClick;
    ok('the validator answers the four gestures a reader makes: paste and click, click on nothing, choose a file, click the drop zone',
      /* PAS UN MOT ISOLE, LE VERDICT. Une premiere version acceptait le mot
       * « question » : remplacer toute l'analyse par out.textContent = 'question'
       * la laissait verte. On exige la phrase de tete que la page rend et le
       * decompte qu'elle calcule. */
      apresColle.indexOf(PAGE.S.counts.split('{')[0].trim()) >= 0 &&
      (apresColle.indexOf('publiable') >= 0 || apresColle.indexOf('publishable') >= 0 ||
       apresColle.indexOf('exemple') >= 0 || apresColle.indexOf('example') >= 0) &&
      apresVide === 'TEMOIN' &&
      apresFichier.length > 0 && apresFichier.indexOf('3.1.11') >= 0 &&
      ouvert === 1,
      `collé ${apresColle.length}, vide « ${apresVide} », fichier ${apresFichier.length}, zone ${ouvert}`);
    /* On rend le shim dans l'etat ou on l'a trouve : un faux fichier reste dans le
     * selecteur faisait mourir une section suivante, le vrai FileReader n'existant
     * pas sous Node. */
    ids.paste.value = '';
    delete ids.file.files;
    ids.out.textContent = '';
    PAGE.analyse(html);
  }

  /* ---- LA PHRASE DE TETE DU VALIDATEUR ------------------------------------
   * C'est la seule ligne que beaucoup de gens liront de cette page, et elle
   * etait calculee au milieu du rendu, entre deux appendChild, hors d'atteinte
   * de toute assertion. Elle est desormais une fonction pure, et voici son
   * ordre, cas par cas, dans les deux langues.
   *
   * LE CAS QUI A MOTIVE TOUT CECI : quelqu'un telecharge ai-go.html, le fichier
   * que le README appelle « le moteur », le depose ici, et recoit « Cohérent,
   * mais NON publiable : 13 point(s) à compléter » au-dessus d'un tableau rouge
   * de treize lignes. Le fichier n'est pas casse : c'est le GABARIT VIERGE, et
   * treize blancs, c'est ce qu'un gabarit vierge doit avoir. La phrase qui
   * l'expliquait etait la treizieme ligne du tableau. */
  {
    const avant = PAGE.lang();
    const tete = (langue, etat) => { PAGE.setLang(langue); return PAGE.verdictOf(etat); };
    const VIDE = { crashed: [], errors: 0, engineMissing: false, guard: 0, blank: false, warns: 0, truncated: false };
    /* CHAQUE CAS EXIGE LA BONNE PHRASE. Une premiere version acceptait, pour le
     * cas « sans moteur », la seule ABSENCE de la phrase de l'exemple : un
     * verdict vide l'aurait satisfaite. On demande donc toujours ce qui doit
     * etre la, et en plus, quand c'est demande, ce qui ne doit pas y etre. */
    const cas = (nom, etat, classe, frDoit, enDoit, frInterdit, enInterdit) => {
      const fr = tete('fr', Object.assign({}, VIDE, etat));
      const en = tete('en', Object.assign({}, VIDE, etat));
      const bon = fr.cls === classe && en.cls === classe &&
        fr.head.indexOf(frDoit) >= 0 && en.head.indexOf(enDoit) >= 0 &&
        frDoit !== '' && enDoit !== '' &&
        (frInterdit === undefined || (fr.head.indexOf(frInterdit) < 0 && en.head.indexOf(enInterdit) < 0));
      return bon ? null : nom;
    };
    const rates = [
      cas('un fichier dont un script a plante', { crashed: ['boum'] }, 'v-bad', 'échoué', 'failed'),
      cas('des erreurs bloquantes', { errors: 3 }, 'v-bad', '3', '3'),
      /* Un gabarit vierge dont le moteur manque se dit d'abord sans moteur : la
       * phrase du gabarit ne doit pas masquer un fichier ampute. */
      /* « moteur » et « engine » sont dans les DEUX phrases : « le gabarit vierge
       * livré avec le moteur » les contient aussi. Un fragment qui colle a la
       * mauvaise reponse ne teste rien. On exige donc la phrase du moteur absent
       * ET l'absence de celle du gabarit. */
      cas('un fichier sans moteur', { engineMissing: true, guard: 13, blank: true }, 'v-warn',
        'moteur', 'engine', 'd’exemple', 'example questionnaire'),
      cas('le questionnaire d’exemple livre avec ce depot', { guard: 13, blank: true }, 'v-warn', 'd’exemple', 'example questionnaire'),
      cas('un contenu a soi, encore incomplet', { guard: 2, blank: false }, 'v-warn', 'NON publiable', 'NOT publishable'),
      /* UNE ENUMERATION TRONQUEE NE SE CONCLUT PAS, et elle passe AVANT les
       * reserves : on ne dit pas « publiable avec 4 avertissements » d'un
       * fichier qu'on n'a pas fini de lire. */
      cas('une enumeration tronquee', { truncated: true, warns: 4 }, 'v-warn',
        'Pas de verdict', 'No verdict', 'Publiable', 'Publishable'),
      cas('un contenu publiable avec des reserves', { warns: 4 }, 'v-warn', 'Publiable', 'Publishable'),
      /* indexOf('') est toujours vrai : ce cas ne comparait aucun texte. */
      cas('un contenu publiable sans reserve', {}, 'v-ok', 'Publiable', 'Publishable')
    ].filter(Boolean);
    PAGE.setLang(avant);
    ok('the validator head names the right one of its 7 verdicts, in both languages',
      rates.length === 0, rates.join(' | '));
    /* ET LE GABARIT SE RECONNAIT A L'IDENTIFIANT QUE CE DEPOT LIVRE, pas a son
     * nombre de blancs : une institution qui a pose le sien retrouve le verdict
     * ordinaire meme si elle en a treize a remplir. */
    PAGE.setLang('fr');
    const memeCompte = PAGE.verdictOf(Object.assign({}, VIDE, { guard: 13, blank: false })).head;
    PAGE.setLang(avant);
    ok('and a questionnaire of one’s own gets the ordinary verdict, thirteen blanks or not',
      memeCompte.indexOf('exemple') < 0 && memeCompte.indexOf('13') >= 0, memeCompte);
    /* ET LE SIGNAL EST LE JUGEMENT DU MOTEUR, pas un second jugement ecrit ici.
     * Le moteur reconnait son propre exemple par empreinte et pose sa remarque
     * sur `nodes` ; cette page la lit, et rien d'autre. */
    ok('the validator reads the engine’s own judgement on the questions, and does not judge them again',
      PAGE.stillTheExample([{ path: 'nodes', message: 'x' }]) === true &&
      PAGE.stillTheExample([{ path: 'publisher.name', message: 'x' }]) === false &&
      PAGE.stillTheExample([]) === false && PAGE.stillTheExample(null) === false);
  }


  /* ---- DANS QUELLE LANGUE CETTE PAGE S'OUVRE ------------------------------
   * check.html exporte `preferred` avec un commentaire qui promet cette
   * epreuve : « Exported so the harness can assert the rule rather than a
   * screenshot of it ». Le harnais ne le mentionnait nulle part. La regle la
   * plus visible de la page -- la langue dans laquelle un lecteur la trouve en
   * arrivant -- n'etait donc tenue par rien.
   *
   * Le documentElement du faux DOM n'a pas de getAttribute : sans celui-ci,
   * preferred() retombe sur son premier garde-fou et rend 'en' quoi qu'on lui
   * demande, ce qui aurait fait passer n'importe quelle assertion. */
  {
    const vraiDoc = globalThis.document.documentElement;
    const vraiNav = Object.getOwnPropertyDescriptor(globalThis, "navigator");
    /* navigator est en lecture seule sur les moteurs recents : on le pose par
     * descripteur, et on rend exactement celui qui etait la. */
    const poseNav = v => Object.defineProperty(globalThis, "navigator", { value: v, configurable: true, writable: true });
    const racine = lang => ({ lang: lang, getAttribute: k => (k === 'lang' ? lang : null), setAttribute() {} });
    const lit = (demandees, langueDeLaPage) => {
      globalThis.document.documentElement = racine(langueDeLaPage);
      poseNav({ languages: demandees, language: demandees[0] });
      try { return PAGE.preferred(); } finally {
        globalThis.document.documentElement = vraiDoc;
        if (vraiNav) Object.defineProperty(globalThis, "navigator", vraiNav); else delete globalThis.navigator;
      }
    };
    const CAS = [
      ['un lecteur francophone', ['fr'], 'fr', 'fr'],
      ['un lecteur anglophone', ['en'], 'fr', 'en'],
      ['le français de Suisse, région comprise', ['fr-CH', 'fr'], 'fr', 'fr'],
      ['l’anglais britannique', ['en-GB'], 'fr', 'en'],
      ['l’anglais demandé en second', ['de-DE', 'en'], 'fr', 'en'],
      /* NI L'UN NI L'AUTRE. Un poste alémanique ou tessinois ne demande aucune
       * des deux langues de cette page : elle s'ouvre alors dans la sienne,
       * celle que son propre balisage déclare, et les deux boutons restent. */
      ['un poste alémanique', ['de-CH', 'de'], 'fr', 'fr'],
      ['un poste tessinois', ['it-CH', 'it'], 'fr', 'fr'],
      ['le même poste devant une page écrite en anglais', ['de-CH'], 'en', 'en'],
      ['aucune langue demandée du tout', [], 'fr', 'fr']
    ];
    const mal = CAS.filter(([, d, pageLang, veut]) => lit(d, pageLang) !== veut);
    ok(`the validator opens in the right language for the ${CAS.length} readers tried`,
      mal.length === 0, mal.map(x => x[0] + ' -> ' + lit(x[1], x[2])).join(' | '));
    /* ET LE CHOIX FAIT L'EMPORTE SUR LE NAVIGATEUR : deux boutons ne servent à
     * rien si la page les oublie au rechargement suivant. */
    const magasin = globalThis.window.sessionStorage;
    const garde = {};
    globalThis.window.sessionStorage = { getItem: k => (k in garde ? garde[k] : null), setItem: (k, v) => { garde[k] = v; }, removeItem: k => { delete garde[k]; } };
    garde['aigo:check:lang'] = 'en';
    const retenu = lit(['fr'], 'fr');
    garde['aigo:check:lang'] = 'zz';
    const absurde = lit(['fr'], 'fr');
    globalThis.window.sessionStorage = magasin;
    ok('a language the reader picked wins over the one the browser asks for',
      retenu === 'en', String(retenu));
    ok('and a remembered value that is not one of the two is ignored, not served',
      absurde === 'fr', String(absurde));
  }
  ...
  /* CHAQUE IDENTIFIANT DEMANDE PAR LE SCRIPT EXISTE DANS LE BALISAGE. Le faux DOM
   * rend ce qu'on lui demande, donc renommer id="pasteGo" laissait le harnais tout
   * vert pendant que le vrai navigateur levait « Cannot read properties of null »
   * et ne rendait aucun verdict. C'est un noteur externe qui l'a montre. */
  {
    const script = bodiesOf(checkHtml)[0];
    const balisage = PAGE.markupOnly ? PAGE.markupOnly(checkHtml) : checkHtml.replace(/<script[\s\S]*?<\/script>/g, '');
    const demandes = [...script.matchAll(/getElementById\('([a-zA-Z][\w-]*)'\)/g)].map(m => m[1]);
    /* getElementById('lang-' + code) est calcule : les deux codes de la page. */
    const calcules = /getElementById\('lang-' \+ code\)/.test(script) ? ['lang-fr', 'lang-en'] : [];
    const voulus = [...new Set(demandes.concat(calcules))];
    const absents = voulus.filter(id => balisage.indexOf('id="' + id + '"') < 0);
    /* UN TYPE QUI N'EST PAS DU JAVASCRIPT REND LE SCRIPT INERTE dans un
     * navigateur, pendant que ce harnais en extrait le texte et l'execute quand
     * meme : la page etait morte et la course verte. Les types admis sont ceux
     * qu'un navigateur execute. */
    /* LE TYPE ET LES ATTRIBUTS QUI RENDENT MUET. `nomodule` suffit a ce qu'un
     * navigateur moderne n'execute rien, et ce harnais executait quand meme le
     * corps qu'il avait extrait : page morte, course verte. */
    const muets = [...checkHtml.matchAll(/<script([^>]*)>/g)].map(m => m[1]).map(attrs => {
      const t = (/type\s*=\s*["']([^"']+)["']/.exec(attrs) || [])[1];
      if (t && !/^(text\/javascript|application\/javascript|module)$/i.test(t)) return 'type=' + t;
      if (/\bnomodule\b/.test(attrs)) return 'nomodule';
      if (/\bdefer\b/.test(attrs) && !/\bsrc\b/.test(attrs)) return 'defer sans src';
      return null;
    }).filter(Boolean);
    ok('check.html carries no script a browser would refuse to run', muets.length === 0, muets.join(', '));
    ok(`check.html holds the ${voulus.length} element ids its own script asks for by name`,
      voulus.length >= 8 && absents.length === 0, absents.join(', '));
    /* ET AUCUN N'EST DESACTIVE. Ajouter `disabled` a #pasteGo passait le harnais,
     * qui ne verifiait que la presence de l'identifiant ; le bouton etait pourtant
     * incliquable dans le navigateur. */
    const eteints = voulus.filter(id => {
      const t = new RegExp('<[^>]*id="' + id + '"[^>]*>').exec(balisage);
      return t && /\bdisabled\b/.test(t[0]);
    });
    /* UN FIELDSET DESACTIVE ETEINT CE QU'IL CONTIENT : ne regarder que l'element
     * laissait passer un bouton eteint par son parent. Aucun attribut `disabled`
     * n'a de raison d'etre dans cette page, nulle part. */
    const partout = (balisage.match(/<[^>]*\sdisabled[\s>=]/g) || []).length;
    ok('and none of them is delivered disabled, by itself or by an ancestor',
      eteints.length === 0 && partout === 0, eteints.join(', ') + ' | ' + partout + ' attribut(s) disabled');
  }
  /* UNE AIDE NON TRADUITE, QUAND C'EST UNE LISTE. Le balayage ne se declenchait
   * que sur une chaine nue. Une aide s'ecrit aussi en tableau de paragraphes et
   * de listes a puces : `help: ['Texte francais', { items: [...] }]` dans un arbre
   * FR/EN donnait zero avertissement et verdict vert, pendant que l'interface
   * anglaise servait cette aide en francais. Trouve par un noteur externe. */
  {
    const arbre = lang => ({
      id: 'x', version: '1.0.0', langs: ['fr', 'en'], defaultLang: 'fr', start: 'q1',
      publisher: { name: 'H', domains: ['h.ch'] }, review: { by: 'S', date: '2026-09-01' },
      jurisdiction: 'CH-VD', legalBasis: ['LPD'], disclaimer: { fr: 'a', en: 'b' },
      steps: [{ id: 's1', name: { fr: 'E', en: 'S' } }],
      nodes: { q1: { type: 'single', step: 's1', title: { fr: 'T', en: 'T' }, help: lang,
        answers: [{ value: 'yes', label: { fr: 'Oui', en: 'Yes' }, result: 'r' },
                  { value: 'no', label: { fr: 'Non', en: 'No' }, result: 'r' }] } },
      results: { r: { level: 'info', step: 's1', title: { fr: 'R', en: 'R' }, solution: { text: { fr: 's', en: 's' } } } },
      links: {},
    });
    const vus = h => PAGE.extras(arbre(h), PAGE.paths(arbre(h))).filter(i => /help/.test(i.where)).map(i => i.where);
    const enListe = vus(['Texte français', { items: ['Puce française'] }]);
    const enChaine = vus('Texte français');
    const traduite = vus({ fr: ['Aide'], en: ['Help'] });
    ok('the validator names an untranslated help whether it is a bare string or a list of paragraphs and bullets',
      enChaine.length === 1 && enListe.length === 2 && traduite.length === 0,
      `chaîne ${enChaine.length}, liste ${enListe.length}, traduite ${traduite.length}`);
  }
  ...
  /* UNE PAGE QUE LE NAVIGATEUR LAISSERA BLANCHE N'EST PAS PUBLIABLE. Le moteur
   * est un script en ligne : une adaptation ou 'unsafe-inline' avait saute du
   * script-src recevait « Publiable », et Chromium montrait un conteneur vide.
   * Cette page lisait le contenu sans jamais eprouver la politique d'execution
   * du fichier. Trouve par un noteur externe sur la version publiee. */
  {
    const sansEnLigne = html.replace("script-src 'unsafe-inline'", "script-src 'none'");
    const cas = [
      ['le fichier livré', html, 0],
      ['sans scripts en ligne', sansEnLigne, 1],
      ['sans aucune CSP', html.replace(/<meta http-equiv="Content-Security-Policy"[^>]*>/, ''), 0],

      /* Viser LA balise de politique, pas le premier attribut content= du fichier. */
      ['hérité de default-src', html.replace(/(<meta http-equiv="Content-Security-Policy"[^>]*content=)"[^"]*"/, '$1"default-src ' + "'none'" + '"'), 1],
      /* UN NONCE N'AUTORISE QUE LES SCRIPTS QUI LE PORTENT, et script-src-elem
       * prime sur script-src pour les balises : deux trous de ma premiere version,
       * montres par un noteur avec Chrome a l'appui. */
      ['un nonce que personne ne porte', html.replace("script-src 'unsafe-inline'", "script-src 'nonce-abc'"), 1],
      ['script-src-elem qui bloque', html.replace("script-src 'unsafe-inline'", "script-src 'unsafe-inline'; script-src-elem 'none'"), 1],
      /* LA SPECIFICATION, PAS NOTRE CONVENANCE : des qu'un nonce figure, le
       * navigateur ignore 'unsafe-inline'. Les accepter ensemble rendait vert une
       * page que Chrome laisse blanche. */
      ['unsafe-inline accompagné d’un nonce', html.replace("script-src 'unsafe-inline'", "script-src 'unsafe-inline' 'nonce-abc'"), 1],
      /* LA PREMIERE DIRECTIVE GAGNE, comme dans la specification. */
      ['deux script-src, la première interdisant', html.replace("script-src 'unsafe-inline'", "script-src 'none'; script-src 'unsafe-inline'"), 1],
      /* Deux politiques se croisent : la plus stricte gagne. */
      ['une seconde politique qui interdit', html.replace('<meta http-equiv="Content-Security-Policy"',
        '<meta http-equiv="Content-Security-Policy" content="script-src none"><meta http-equiv="Content-Security-Policy"'), 1],
    ].filter(([, t, n]) => PAGE.cspIssues(t).length !== n).map(([nom]) => nom);
    ok('the validator refuses a file whose own policy would stop its engine from running',
      cas.length === 0 && sansEnLigne !== html, cas.join(', '));
    /* ET ELLE S'APPLIQUE SA PROPRE REGLE. Remplacer la CSP de check.html par
     * script-src 'none' laissait le harnais vert, avec AI_GO_CHECK indefini dans
     * le navigateur : la page qui refuse un fichier inexecutable ne se regardait
     * pas elle-meme. */
    ok('and check.html holds itself to it: its own policy lets its own scripts run',
      PAGE.cspIssues(checkHtml).length === 0 &&
      PAGE.cspIssues(checkHtml.replace("script-src 'unsafe-inline'", "script-src 'none'")).length === 1,
      PAGE.cspIssues(checkHtml).map(i => i.message).join(' | '));
  }
  /* ---- UNE PAGE QU'ON N'A PAS FINI DE LIRE NE SE DIT PAS PUBLIABLE --------
   * L'enumeration des chemins s'arrete a une borne. Le drapeau existait et
   * s'ecrivait dans un paragraphe tout en bas ; la phrase de tete, celle que
   * beaucoup liront seule, disait « Publiable, rien a signaler ». Deux noteurs
   * externes ont trouve ce defaut le meme jour, chacun de son cote. */
  {
    const profond = { id: 'trop-profond', version: '1.0.0', langs: ['fr'], defaultLang: 'fr', start: 'q1',
      publisher: { name: 'Haute ecole', domains: ['heg-exemple.ch'] },
      review: { by: 'Service juridique', date: '2026-09-01' },
      jurisdiction: 'CH-VD', legalBasis: ['LPD'],
      disclaimer: { fr: 'Cet outil ne remplace pas un avis juridique.' },
      steps: [{ id: 's1', name: { fr: 'Etape' } }], nodes: {}, results: {}, links: {} };
    for (let n = 1; n <= 700; n++) {
      profond.nodes['q' + n] = { type: 'single', step: 's1', title: { fr: 'Question ' + n },
        answers: [{ value: 'yes', label: { fr: 'Oui' }, to: n < 700 ? 'q' + (n + 1) : undefined, result: n < 700 ? undefined : 'fin' },
          { value: 'no', label: { fr: 'Non' }, result: 'fin' }] };
    }
    profond.results.fin = { level: 'info', step: 's1', title: { fr: 'Fin' }, solution: { text: { fr: 'Rien a signaler.' } } };
    const enumere = PAGE.paths(profond);
    const avant = PAGE.lang();
    PAGE.setLang('fr');
    const tete = PAGE.verdictOf({ crashed: [], errors: 0, engineMissing: false, guard: 0,
      blank: false, warns: 0, truncated: !!enumere.truncated }).head;
    PAGE.setLang(avant);
    ok('a tree whose paths cannot all be enumerated is not called publishable',
      enumere.truncated === true && tete.indexOf('Pas de verdict') >= 0 && tete.indexOf('Publiable') < 0, tete);
    /* ET LA COMPARAISON DE MISE A JOUR NE DIT PLUS « inchange » sur une
     * enumeration tronquee : routingDiff portait deja le drapeau, avec un
     * commentaire disant qu'il invalide la comparaison, et l'affichage l'ignorait. */
    const d = PAGE.routingDiff(profond, profond);
    ok('and the update comparison reports the two enumerations as incomparable, not as unchanged',
      d.truncated === true && d.added.length === 0 && d.removed.length === 0,
      JSON.stringify({ t: d.truncated, u: d.unchanged }));
  }

  /* ---- CE QUE VOIT QUI DEPOSE LE FICHIER VEDETTE DE CE DEPOT ---------------
   * Le fichier que le README appelle « le moteur » est le GABARIT VIERGE, et il
   * rend treize points a completer : c'est ce qu'un gabarit vierge doit rendre.
   * La page le disait a la treizieme ligne d'un tableau rouge. Elle le dit
   * maintenant en tete, et ces deux assertions passent par le vrai chemin -- le
   * fichier livre entre, la phrase que le visiteur lit sort -- parce que les
   * assertions qui appelaient verdictOf avec blank deja calcule ne touchaient
   * jamais le cablage entre tree.id et blank, et un mutant qui reconnaissait le
   * gabarit a son NOMBRE de blancs leur survivait. */
  if (onlyDelivered('the head a visitor gets when dropping the shipped file')) {
    /* LE FAIT BLOQUANT RESTE DANS LA PHRASE. Une premiere version rassurait si
     * bien qu'elle avait laisse tomber « NON publiable » : expliquer pourquoi un
     * fichier est rouge ne doit pas effacer qu'il ne peut pas partir. */
    ok('the validator calls the shipped file an example instead of scolding it thirteen times, and still says it cannot be published',
      shown.includes('example questionnaire shipped with the engine') && shown.includes('NOT publishable'),
      shown.slice(0, 140));
    /* CHANGER LE SEUL IDENTIFIANT NE FAIT PAS UN QUESTIONNAIRE A SOI. C'est le
     * defaut qu'un audit externe a trouve dans la premiere version de cette
     * correction : une institution qui avait ecrit ses onze questions et oublie
     * l'identifiant s'entendait dire que le fichier attendait son contenu. */
    const renomme = html.replace('id: "my-questionnaire"', 'id: "hes-donnees"');
    PAGE.analyse(renomme);
    const apresRenommage = ids.out.textContent;
    ok('renaming the file alone does not make it your questionnaire: the example is still the example',
      renomme !== html && apresRenommage.includes('example questionnaire shipped with the engine'),
      apresRenommage.slice(0, 140));
    /* REMPLACER LES QUESTIONS, OUI. Un seul intitule suffit a changer l'empreinte
     * que le moteur compare, et le verdict redevient l'ordinaire. */
    const sien = html.replace('en: "Can your data be traced back to a person?"',
      'en: "Does your dataset name a student?"');
    PAGE.analyse(sien);
    const seen = ids.out.textContent;
    ok('but replacing the questions does: a questionnaire of one’s own gets the ordinary verdict',
      sien !== html && seen.includes('NOT publishable') &&
      !seen.includes('example questionnaire shipped with the engine'),
      seen.slice(0, 140));
    /* ET LE TROISIEME CAS, celui qui separe vraiment les deux regles : on remplit
     * deux blancs sans toucher a l'identifiant. Le compte tombe sous treize, et
     * c'est toujours le gabarit livre. Sans ce cas, reconnaitre le gabarit « a
     * partir de treize points a completer » passait les deux assertions
     * ci-dessus, parce que changer l'identifiant en retire precisement un. */
    const started = html.replace('publisher: { name: "TO_FILL_IN", domains: ["TO_FILL_IN"] }',
      'publisher: { name: "Haute ecole de gestion", domains: ["heg-exemple.ch"] }');
    PAGE.analyse(started);
    const early = ids.out.textContent;
    ok('a half-filled example is still the example, recognised by its questions and not by its count of blanks',
      started !== html && early.includes('example questionnaire shipped with the engine'),
      early.slice(0, 140));
    PAGE.analyse(html);
  }
  ...
  /* ET SUR LE SECOND FICHIER AUSSI. Une personne verifie un fichier, puis en
   * depose un autre : la section de mise a jour compare les deux routages, et
   * elle relit le nouvel arbre hors du garde qui protege tout le reste. Un arbre
   * qui leve a la lecture arretait donc la page entiere, sans banniere et sans un
   * mot -- mais seulement au deuxieme depot, ce qu'aucune assertion n'atteignait
   * puisque aucune ne deposait deux fois. */
  PAGE.analyse(html);
  PAGE.analyse(crashing);
  ok('and it still says so when it is the second file checked, where the update comparison reads the tree again',
    /The check failed/.test(ids.out.textContent), ids.out.textContent.slice(0, 90));
  ...
  /* ET C'EST LE GESTE QUE LE README PROMET. Une mesure a releve que le vrai
   * questionnaire n'etait executable nulle part : on pouvait le lire, pas
   * l'essayer, et le fichier qu'on double-clique porte un exemple de trois
   * questions. Le harnais composait cette page depuis toujours sans le dire a
   * personne ; le README le dit maintenant, et les deux sont tenus ensemble. */
  {
    const dit = read('README.md');
    const pieces = ['AI_GO-CONTENT-BEGIN', 'AI_GO-CONTENT-END', 'reference/aigo-unil.js',
      'cbdb4863aed9f778', '20 chemins'].filter(x => dit.indexOf(x) < 0);
    ok('the README tells the reader the one paste that gives them the real questionnaire to try, and names what it must yield',
      pieces.length === 0 && /essayer le questionnaire de l’UNIL en local/.test(dit),
      pieces.join(', '));
  }
  }
});
...
if (isThisRepository) section(() => {
  ...
  /* CENT QUINZE, ET NON CENT DIX. La borne dit « une page », et elle est un
   * garde-fou contre l'enflure, pas une fin. Elle est passee de 110 a 115 le
   * jour ou une mesure faite par des visiteurs non techniques a demande trois
   * ajouts que ce README n'avait pas : l'avertissement « ce n'est pas un avis
   * juridique », la contradiction de licence du billet cite en tete, et le
   * registre ou verifier l'empreinte d'une copie. Quatre lignes de gras avaient
   * ete coupees ailleurs d'abord ; ce qui restait etait porteur. Si ce nombre
   * doit remonter encore, coupez avant de le lever.
   *
   * CENT DIX-SEPT, ET NON CENT QUINZE. Meme motif, meme discipline : une mesure
   * faite par huit noteurs a designe l'accueil comme l'axe le plus faible, et
   * nomme trois manques -- ce que ce depot EST (ni service en ligne ni
   * bibliotheque), comment essayer le questionnaire de l'UNIL en local au lieu
   * de l'exemple de trois questions, et cinq sigles lances sans un mot. Cinq
   * paragraphes ont ete replies avant d'y toucher, sans perdre un mot. */
  ok(`README is one page (${lines} lines, limit 117)`, lines <= 117, String(lines));
  /* ET LA PAGE QUI ENSEIGNE LA REGLE DIT LA MEME. Elle annoncait 110 quand la
   * borne etait passee a 115 : qui contribue lisait une regle que le depot ne
   * suit pas, et rien ne les tenait ensemble. */
  const saidLimit = (read('CONTRIBUTING.md').match(/README d’au plus (\d+) lignes/) || [])[1];
  ok('CONTRIBUTING.md states the same README limit the harness enforces',
    Number(saidLimit) === 117, `CONTRIBUTING dit ${saidLimit}, le harnais 117`);
  ...
  /* LES GESTES, PAS DEUX MOTS. Cette assertion ne cherchait que « check.html » et
   * « fingerprint » n'importe ou dans le guide : supprimer tout le paragraphe
   * « Updating the engine » la laissait verte. Un adoptant qui met a jour sans
   * cette procedure ecrase son propre contenu avec l'exemple livre. */
  {
    const gestes = [
      ['le titre de la procédure', /\*\*Updating the engine\.\*\*/],
      ['la région à reprendre', /from the first line to the line before/],
      ['le marqueur qui la borne', /AI_GO-ENGINE-BEGIN/],
      ['le geste lui-même', /paste it over the same region of the new file/],
      ['les deux moitiés du reçu', /receipt has two halves/],
      ['la page qui vérifie', /check\.html/],
      ['l’empreinte du contenu', /content fingerprint/i],
    ].filter(([, re]) => !re.test(guide)).map(([nom]) => nom);
    ok(`the guide spells out the ${7 - gestes.length}/7 steps of the update procedure, not just the words around it`,
      gestes.length === 0, gestes.join(', '));
  }

  /* LE FRAGMENT QUE LE GUIDE FAIT RECOPIER, EXECUTE. Le remplacer par `void 0`
   * laissait le harnais vert : il cherchait trois fragments de texte. Dans un
   * navigateur, l'iframe gardait sa hauteur apres un message en demandant une
   * autre. On lit donc le fragment dans le guide et on le fait tourner. */
  {
    const bloc = (guide.match(/```js\n([\s\S]*?)```/) || [])[1] || '';
    const cadre = { style: { height: '150px' }, dataset: { aigo: '' } };
    let ecoute = null;
    const faux = {
      addEventListener: (nom, fn) => { if (nom === 'message') ecoute = fn; },
      /* LE SELECTEUR EXACT, pas « tout ce qui le contient » : « .absent
       * iframe[data-aigo] » passait, alors qu'un navigateur ne trouve rien. */
      document: { querySelector: sel => (String(sel).trim() === 'iframe[data-aigo]' ? cadre : null) },
    };
    let pose = false;
    try {
      new Function('addEventListener', 'document', bloc)(faux.addEventListener, faux.document);
      if (ecoute) ecoute({ data: { aigo: 'height', px: 900 } });
      pose = cadre.style.height === '900px';
      if (ecoute) ecoute({ data: { aigo: 'autre', px: 42 } });
    } catch (e) { pose = false; }
    ok('the iframe snippet the guide hands out actually resizes the frame, and only on the engine’s own message',
      bloc.length > 0 && pose && cadre.style.height === '900px', 'hauteur ' + cadre.style.height);
  }

  /* ET CETTE PROCEDURE MARCHE VRAIMENT POUR QUI N'A PRIS QUE CES FICHIERS. Le
   * guide promet « it checks what it still can » ; le harnais mourait sur une
   * trace de pile ENOENT a la premiere page absente, sans un seul FAIL, et le
   * mode reduit qu'il porte etait du code mort pour l'adoptante. On le relance
   * donc dans un dossier qui ne contient que ce que le guide fait telecharger. */
  {
    const nu = mkdtempSync(join(tmpdir(), 'aigo-nu-'));
    try {
      mkdirSync(join(nu, 'test'), { recursive: true });
      ['ai-go.html', 'check.html', 'LICENSE'].forEach(f => writeFileSync(join(nu, f), read(f), 'utf8'));
      writeFileSync(join(nu, 'test/run.mjs'), read('test/run.mjs'), 'utf8');
      let sortie = '', code = 0;
      try { sortie = execFileSync(process.execPath, [join(nu, 'test/run.mjs')], { encoding: 'utf8', stdio: 'pipe' }); }
      catch (e) { code = e.status; sortie = String(e.stdout || '') + String(e.stderr || ''); }
      const dit = (sortie.match(/(\d+)\/(\d+) passed/) || []);
      ok('the harness run by an institution that took only the files the guide names finishes, and names what it did not read',
        code === 0 && dit[1] === dit[2] && Number(dit[1]) > 400 && /This run was reduced/.test(sortie) &&
        /docs\/DEMARRER\.md/.test(sortie),
        'exit ' + code + ' ' + (dit[0] || sortie.split('\n').slice(-3).join(' ')));
    } finally { rmSync(nu, { recursive: true, force: true }); }
  }
  ...
  const OWN_FILES = ['/.github/workflows/test.yml', '/.gitignore', '/CHANGELOG.md',
    ...
    '/docs/INTEGRATE.md', '/docs/PUBLICATIONS.md', '/docs/TREE.en.md', '/docs/capture.png',
    '/reference/aigo-unil.js', '/reference/aigo-unil.paths.js', '/test/run.mjs',
    '/tools/anglais.mjs', '/tools/typographie.mjs'];
  ...
  /* LE TIRET CADRATIN EST NOTRE REGLE, PAS CELLE D'UNE ADOPTANTE. Le meme garde
   * que la verification page par page porte deja : sur un fichier dont le contenu
   * n'est plus le notre, cette regle de style ne s'applique plus, faute de quoi le
   * harnais rougit sur la typographie d'une autre institution alors que
   * docs/INTEGRATE.md promet qu'il verifie « ce qu'il peut encore ». */
  const styled = asDelivered ? text : text.filter(p => p !== 'ai-go.html');
  if (!asDelivered) skipped.push('la typographie de ai-go.html, dont le contenu n’est pas le nôtre');
  const dashed = styled.filter(p => read(p).includes(EM_DASH));
  ok(`no em dash in any of the ${styled.length} text files whose text is ours`, dashed.length === 0, dashed.join(', '));
  ...
  /* ---- LA PAGE ANGLAISE, RENDUE ET NON TRADUITE ---------------------------
   * docs/TREE.en.md donne le questionnaire a qui ne lit pas le francais. Pas un
   * mot n'y est traduit : chaque question, case, reponse, resultat et mise en
   * garde est la chaine en: de reference/aigo-unil.js, l'anglais de
   * l'institution, deja publie dans ce depot. Ce que tools/anglais.mjs ajoute de
   * sa main, ce sont les titres de section, les connecteurs de routage et les
   * trois paragraphes de tete -- rien de juridique.
   *
   * Quatre epreuves, et la quatrieme est celle qui compte : l'outil PROMET dans
   * son en-tete que rien ne retombe en francais en silence. Une promesse que le
   * harnais ne peut pas eprouver est le defaut qu'on a vu passer tout le jour. */
  {
    const TOOL_EN = 'tools/anglais.mjs';
    const page = read('docs/TREE.en.md');
    /* 1. LA PAGE EST CE QUE L'ARBRE REND, octet pour octet. On lance le script
     *    livre, jamais une copie de ses regles ici : une implementation
     *    parallele passerait pendant que le fichier livre est faux. */
    let code = 0, said = '';
    try { execFileSync(process.execPath, [join(ROOT, TOOL_EN), '--check'], { encoding: 'utf8', stdio: 'pipe' }); }
    catch (e) { code = e.status; said = String(e.stderr || ''); }
    ok('docs/TREE.en.md is exactly what the frozen tree renders, byte for byte',
      code === 0, 'exit ' + code + ' ' + said.trim());
    /* 2. LES DEUX CONSTANTES QUE L'OUTIL ECRIT sont celles du moteur. Une page
     *    qui annonce une empreinte fausse est pire qu'une page sans empreinte :
     *    une premiere version recalculait l'empreinte a la main et rendait
     *    f1316c44b78035d2 la ou le moteur rend cbdb4863aed9f778. */
    ok('the English page cites the engine version and the content fingerprint the engine itself computes',
      page.includes(AI_GO.fingerprint(unil)) && page.includes(AI_GO.version),
      'empreinte ' + (page.includes(AI_GO.fingerprint(unil)) ? 'ok' : AI_GO.fingerprint(unil)) +
      ', version ' + (page.includes(AI_GO.version) ? 'ok' : AI_GO.version));
    /* 3. RIEN N'EST LAISSE DE COTE. Traversee independante : on lit les chaines
     *    en: dans le TEXTE du fichier de reference, pas dans l'objet, de sorte
     *    qu'un generateur qui sauterait un champ entier se fasse prendre ici
     *    meme si la comparaison 1 reste verte, puisqu'elle compare deux sorties
     *    du meme code. */
    /* UNE PREMIERE VERSION LISAIT LES CHAINES A L'EXPRESSION REGULIERE /en: "..."/
     * et en comptait 94. Elle etait aveugle aux six aides dont le `en:` est un
     * TABLEAU : leurs paragraphes et les puces de leurs listes, quinze chaines,
     * n'etaient tenues par rien. La promesse etait trop petite, pas la page. On
     * descend donc dans l'arbre, en ignorant tout ce qui pend sous un `fr`. */
    const englishOf = [];
    const gather = v => {
      if (v == null) return;
      if (typeof v === 'string') { englishOf.push(v); return; }
      if (Array.isArray(v)) { v.forEach(gather); return; }
      if (typeof v === 'object' && v.items) { [].concat(v.items).forEach(gather); }
    };
    const walk = v => {
      if (v == null || typeof v !== 'object') return;
      if (Array.isArray(v)) { v.forEach(walk); return; }
      Object.keys(v).forEach(k => { if (k === 'fr') return; if (k === 'en') gather(v[k]); else walk(v[k]); });
    };
    walk(unil);
    const missing = englishOf.filter(t => page.indexOf(t) < 0);
    ok(`the English page carries all ${englishOf.length} English strings of the frozen tree, not one left out`,
      englishOf.length >= 109 && missing.length === 0 &&
      // et la traversee voit bien les aides en tableau, que la precedente ratait
      englishOf.indexOf('Common medical condition') >= 0,
      missing.map(t => JSON.stringify(t.slice(0, 60))).join(' | '));
    /* 3 bis. LE ROUTAGE, ARETE PAR ARETE, CONTRE L'ARBRE GELE. La comparaison 1
     *    compare deux sorties du meme code : un generateur qui enverrait le
     *    lecteur de q6=yes vers R4 au lieu de R3 la laisserait verte, et la page
     *    anglaise dirait le contraire de la francaise sans qu'une assertion
     *    bouge. Les aretes sont donc recalculees ici depuis l'arbre, dans la
     *    forme que le diagramme emploie, et comparees une a une. */
    const Rnum = {};
    Object.keys(unil.results).forEach((id, i) => { Rnum[id] = 'r' + (i + 1); });
    const vers = to => (unil.nodes[to] ? to : Rnum[to] || to);
    const aretes = [];
    Object.entries(unil.nodes).forEach(([id, n]) => {
      if (n.type === 'multi') {
        aretes.push(`  ${id} -- at least one box --> ${vers(n.next.ifAny || n.next.ifAnyResult)}`);
        aretes.push(`  ${id} -- none --> ${vers(n.next.else || n.next.elseResult)}`);
      } else {
        n.answers.forEach(a2 => {
          const dit = a2.detail && a2.detail.en ? a2.detail.en : (a2.value === 'yes' ? 'yes' : 'no');
          aretes.push(`  ${id} -- ${dit} --> ${vers(a2.to || a2.result)}`);
        });
      }
    });
    const schema = page.slice(page.indexOf('```mermaid'), page.indexOf('```', page.indexOf('```mermaid') + 3));
    const manquantes = aretes.filter(l => schema.indexOf(l + '\n') < 0);
    const dessinees = (schema.match(/^ {2}\w+ -- /gm) || []).length;
    ok(`the English diagram draws the ${aretes.length} edges the frozen tree routes, and not one more`,
      manquantes.length === 0 && dessinees === aretes.length,
      manquantes.join(' | ') + ' | dessinées ' + dessinees);

    /* 4. LA PROMESSE DE L'EN-TETE, EPROUVEE. Un champ sans en: doit arreter le
     *    rendu, pas servir le francais a un lecteur anglophone sans le dire. On
     *    ampute un arbre et on donne le vrai outil a manger. */
    const dir = mkdtempSync(join(tmpdir(), 'aigo-en-'));
    try {
      const maimed = join(dir, 'tree.js');
      writeFileSync(maimed, read('reference/aigo-unil.js').replace(/en: "Are the individuals identifiable\?"/, ''), 'utf8');
      let fell = 0, cried = '';
      try {
        execFileSync(process.execPath, [join(ROOT, TOOL_EN), '--render', maimed,
          join(ROOT, 'reference/aigo-unil.paths.js')], { encoding: 'utf8', stdio: 'pipe' });
      } catch (e) { fell = e.status; cried = String(e.stderr || ''); }
      ok('the renderer stops on a field with no English rather than serve French to an English reader',
        fell === 2 && /no en: at q2/.test(cried), 'exit ' + fell + ' ' + cried.trim());
      /* 5. L'ARBRE D'AUTRUI NE SORT PAS SOUS NOTRE IDENTITE. L'unique epreuve de
       *    --render ne lui soumettait que l'arbre de l'UNIL : la branche qui rend
       *    le tien n'etait exercee par rien, et elle sortait une page titree « The
       *    AI_GO questionnaire of UNIL », portant notre empreinte quelle que soit
       *    la vraie, annoncant vingt chemins pour un arbre qui en a deux, et
       *    signee de notre adresse. Trouve par un noteur externe. */
      const tiers = join(dir, 'tiers.js');
      writeFileSync(tiers, 'module.exports = ' + JSON.stringify({
        id: 'heg-donnees', version: '2026-01-01', langs: ['en'], defaultLang: 'en', start: 'q1',
        publisher: { name: 'Haute ecole de gestion', domains: ['heg-exemple.ch'] },
        review: { by: 'Legal service', date: '2026-01-01' },
        jurisdiction: 'CH-GE', legalBasis: ['LIPAD'],
        disclaimer: { en: 'Not legal advice.' },
        steps: [{ id: 's1', name: { en: 'Nature' } }],
        nodes: { q1: { type: 'single', step: 's1', title: { en: 'Does the data concern a student?' },
          answers: [{ value: 'yes', label: { en: 'Yes' }, result: 'closed' },
                    { value: 'no', label: { en: 'No' }, result: 'open' }] } },
        results: { open: { level: 'info', step: 's1', title: { en: 'Open' }, solution: { text: { en: 'No restriction.' } } },
                   closed: { level: 'stop', step: 's1', title: { en: 'Closed' }, solution: { text: { en: 'Local only.' } } } },
        links: {},
      }) + ';', 'utf8');
      const oracleTiers = join(dir, 'chemins.js');
      writeFileSync(oracleTiers, 'module.exports = ["q1=no => open", "q1=yes => closed"];', 'utf8');
      const rendue = execFileSync(process.execPath,
        [join(ROOT, TOOL_EN), '--render', tiers, oracleTiers], { encoding: 'utf8' });
      const notres = ['UNIL', AI_GO.fingerprint(unil), 'iaunil@unil.ch', 'aigo-unil', '20 possible routes']
        .filter(m => rendue.indexOf(m) >= 0);
      ok('rendering someone else’s tree carries none of our identity: no UNIL, no fingerprint of ours, no address of ours',
        rendue.indexOf('Haute ecole de gestion') >= 0 && rendue.indexOf('2 possible routes') >= 0 && notres.length === 0,
        notres.join(' | '));
    } finally { rmSync(dir, { recursive: true, force: true }); }
  }
  ...
  // already drifted apart by one. Each was written by hand and read by nobody.
  {
    ...
    /* ET LE DEPART DE CHAQUE ENTREE EST L'ARRIVEE DE LA PRECEDENTE. Seul le
     * nombre d'arrivee etait relu, le depart vivant dans un groupe non
     * capturant : trois entrees successives ont pu repartir de 498 pendant que
     * la 3.0.19 annonçait que le harnais « vérifie désormais ses propres
     * chiffres ». Une garantie qui ne couvrait qu'une moitie du chiffre.
     *
     * LES QUATRE ENTREES LES PLUS RECENTES, ET PAS AU-DELA. Sous la 3.0.18, les
     * comptes ont ete ecrits avant que le harnais ne relise le sien, et il y
     * reste un ecart d'une unite : la 3.0.17 arrive a 482, la 3.0.18 repart de
     * 483. Ce sont des chiffres publies que personne ne peut plus remesurer ;
     * on ne les reecrit pas pour faire verdir un test. */
    const suite = read('CHANGELOG.md').replace(/\s+/g, ' ');
    const chiffres = [...suite.matchAll(/passe de (\d+) à (\d+) assertions/g)].map(m => [Number(m[1]), Number(m[2])]);
    const RELUS = 4;
    const casses = chiffres.slice(0, RELUS - 1)
      .map((c, i) => (c[0] === chiffres[i + 1][1] ? null : `${c[0]}≠${chiffres[i + 1][1]}`))
      .filter(Boolean);
    ok(`the ${RELUS} most recent assertion counts chain, each entry starting where the one below ends`,
      chiffres.length >= RELUS && casses.length === 0, casses.join(' | '));

    const CARRIERS = ['README.md', 'CHANGELOG.md', 'CONTRIBUTING.md', 'SECURITY.md',
      'docs/ARBRE.md', 'docs/TREE.en.md', 'docs/PUBLICATIONS.md', 'docs/DEMARRER.md',
      'docs/INTEGRATE.md'];
    ...
    /* ET LA TROISIEME LISTE, celle que l'adoptante a sous les yeux au moment ou
     * elle ecrit sa langue. Le commentaire ci-dessus promet qu'une cle ajoutee au
     * moteur et a une seule liste arrete la course ; deux listes etaient relues,
     * celle-la non. Elle annoncait 33 cles en en nommant 31, et les deux absentes
     * -- record et consultedOn -- sont celles de la feuille imprimee : une
     * institution qui recopiait les cles nommees obtenait de l'anglais sur son
     * papier, sans rien voir a l'ecran, ou ce bloc est cache. */
    if (asDelivered) {
      const passage = html.slice(html.indexOf('ui: { de:'), html.indexOf('le jeu\n  complet est en tête du bloc du moteur.'));
      const mots = new Set(passage.match(/[a-zA-Z][A-Za-z0-9]*/g) || []);
      const tues = Object.keys(AI_GO.UI.en).filter(k => !mots.has(k));
      ok(`the content block names all ${Object.keys(AI_GO.UI.en).length} interface keys where the adopter writes their own`,
        passage.length > 0 && tues.length === 0, tues.join(', '));
    }
