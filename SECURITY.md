...
Les deux disparaissent avec l’onglet et ne circulent pas d’un onglet à l’autre, à une exception près : un onglet ouvert par `window.open` depuis la page reçoit du navigateur une **copie** du `sessionStorage` de son parent, prise à l’ouverture. Ce n’est pas une synchronisation ; les deux divergent ensuite. Le moteur ne les envoie nulle part, puisqu’il
