...
A pointer click used to move to the next question at once, a change of context on input that WCAG 3.2.2 asks to warn
about first; it also sat Continue where a card of the next question would land, so the taught gesture ticked a box
nobody chose. It is gone: selecting never navigates, from either device, and Continue is the only way forward. No
...
as it is written in Switzerland, used to be accepted and read as the twelfth of March. **548 days after
