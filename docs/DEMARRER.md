...
lui, est **au-dessus du premier marqueur**, et non en dessous du dernier. Des six, le moteur ne lit que les attributs du conteneur : les
cinq autres, c’est `check.html` qui les relit. Le moteur, encore plus bas, ne se touche
...
- **Un écart qui a été fermé** : cliquer une réponse à choix unique faisait passer à la question suivante sans
  confirmation, un changement de contexte à la saisie dont le critère **WCAG 3.2.2** demande d’avertir d’abord. Il
  laissait aussi « Continuer » à l’endroit où une carte de la question suivante allait se poser, si bien que le
  geste enseigné cochait une case que personne n’avait choisie. Choisir ne fait plus avancer, d’aucun périphérique.
...
- `review.by` ou `review.date` manquant, ou une date qui n’est pas écrite `AAAA-MM-JJ` – `2026-09-01`,
  éventuellement suivie d’une heure ISO ; `2026-6-1`, avec un mois sur un chiffre, ne l’est pas. Une date suisse, `03.12.2026`, était autrefois acceptée puis lue comme le 12 mars ; elle est
